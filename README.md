...
[XmlInclude(typeof(Bar))] // Method one uses attributes to indicate the inheritors of the interface.
...
{
    Foo foo = new Foo { Id = 1, Bar = new Bar { Id = 2, Value = "Baz" } };
    XmlSerializer<Foo> serializer = new XmlSerializer<Foo>(typeof(Bar)); // Method two passes in the types of the inheritors to the constructor.
    string xml = serializer.Serialize(foo);
    Foo roundTripFoo = serializer.Deserialize(xml);
}
```