...
namespace XSerializer.Tests
{
    public class AttributedPropertyTests
    {
        ...
        public void CustomSerializerThrowsExceptionIfAndOnlyIfBclXmlSerializerDoes(Type type)
        {
            ...
            {
                Assert.That(() => CustomSerializer.GetSerializer(type, TestOptions.Empty), Throws.Nothing, "Type: " + type.Name + Environment.NewLine);
            }
            else
            {
                Assert.That(() => CustomSerializer.GetSerializer(type, TestOptions.Empty), Throws.InstanceOf(thrownException.GetType()), "Type: " + type.Name + Environment.NewLine);
            }
        }
